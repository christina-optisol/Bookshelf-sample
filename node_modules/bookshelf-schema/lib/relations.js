 ...
 *
 */
var BelongsTo, BelongsToMany, Field, Fulfilled, HasMany, HasOne, IntField, MorphMany, MorphOne, MorphTo, Rejected, Relation, StringField, camelize, lowerFirst, pluralize, promiseFinally, pushField, ref, ref1, ref2, singularize, upperFirst, values,
  slice = [].slice,
...
ref2 = require('./utils'), Fulfilled = ref2.Fulfilled, Rejected = ref2.Rejected, promiseFinally = ref2.promiseFinally, values = ref2.values, upperFirst = ref2.upperFirst, lowerFirst = ref2.lowerFirst;

pushField = function(schema, name, field) {
  var f, i, len;
...
Relation = (function() {
  ...
  Relation.prototype._applyThrough = function(builder) {
    var interim, otherKey, otherKeyTarget, throughForeignKey, throughForeignKeyTarget;
    ...
    throughForeignKeyTarget = this.options.throughForeignKeyTarget;
    otherKeyTarget = this.options.otherKeyTarget;
    return function() {
      return builder.call(this).through(interim, throughForeignKey, otherKey, throughForeignKeyTarget, otherKeyTarget);
    };
  };
  ...
  Relation.prototype._deduceAccessorName = function() {
    return "" + (this.option('accessorPrefix', 'relationsAccessorPrefix', '$')) + this.name;
  };
...
HasOne = (function(superClass) {
  ...
  HasOne.prototype._createRelation = function() {
    var foreignKey, foreignKeyTarget, related;
    related = this.relatedModel;
    foreignKey = this.options.foreignKey;
    foreignKeyTarget = this.options.foreignKeyTarget;
    return function() {
      return this.hasOne(related, foreignKey, foreignKeyTarget);
    };
  };
...
BelongsTo = (function(superClass) {
  ...
  BelongsTo.prototype.contributeToSchema = function(schema) {
    ...
    foreignKey = this.options.foreignKey || ((this._relatedModelName().toLowerCase()) + "_id");
    return pushField(schema, foreignKey, IntField(foreignKey));
  };
  ...
  BelongsTo.prototype._createRelation = function() {
    ...
    foreignKeyTarget = this.options.foreignKeyTarget;
    return function() {
      return this.belongsTo(related, foreignKey, foreignKeyTarget);
    };
  };
...
HasMany = (function(superClass) {
  ...
  HasMany.prototype._createRelation = function() {
    ...
    foreignKeyTarget = this.options.foreignKeyTarget;
    return function() {
      return this.hasMany(related, foreignKey, foreignKeyTarget);
    };
  };
...
BelongsToMany = (function(superClass) {
  ...
  BelongsToMany.prototype._createRelation = function() {
    var foreignKey, foreignKeyTarget, otherKey, otherKeyTarget, related, table;
    related = this.relatedModel;
    ...
    otherKeyTarget = this.options.otherKeyTarget;
    return function() {
      return this.belongsToMany(related, table, foreignKey, otherKey, foreignKeyTarget, otherKeyTarget);
    };
  };
...
MorphTo = (function(superClass) {
  ...
  MorphTo.prototype.contributeToSchema = function(schema) {
    ...
    if (this.options.columnNames) {
      ...
      idName = this.polymorphicName + "_id";
      typeName = this.polymorphicName + "_type";
    }
    pushField(schema, idName, IntField(idName));
    return pushField(schema, typeName, StringField(typeName));
  };

  MorphTo.prototype._destroyReject = function(model, options) {
    var polymorphicId, polymorphicType;
    polymorphicId = this.options.columnNames ? this.options.columnNames[0] : this.polymorphicName + "_id";
    polymorphicType = this.options.columnNames ? this.options.columnNames[1] : this.polymorphicName + "_type";
